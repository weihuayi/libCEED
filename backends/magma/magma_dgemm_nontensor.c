...
#define MAGMA_USE_GEMM_SELECTOR
  ...
  magma_queue_t queue )
{
  ...
  if ( nbatch == n) {
    ...
    if ( use_magmablas ) {
      ...
      devblasDgemmStridedBatched(
        magma_queue_get_devblas_handle( queue ),
        devblas_trans_const(transA), devblas_trans_const(transB),
        (int)m, (int)nbatch, (int)k,
